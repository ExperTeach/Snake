...
class Food:
    ...
    def new_pos(self):
        food_x = round(random.randrange(0, self.x_range - self.block_size) / self.block_size) * self.block_size
        food_y = round(random.randrange(0, self.y_range - self.block_size) /self.block_size) * self.block_size
